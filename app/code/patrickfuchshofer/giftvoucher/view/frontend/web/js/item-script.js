jQuery(document).ready(function ($) {
    ...
    $(".wpgv-giftitem-wrapper .wpgv_preview-box:not(:nth-child(2))").addClass(
        "mailhidden"
    );
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $(".voucherValueCard").val(
                    data.special_price ? data.special_price : data.price
                );
                $.each(data.images, function (key, value) {
                    $(
                        ".wpgvstyle" + (parseInt(key) + 1) + " .cardDiv .cardImgTop img"
                    ).attr("src", value);
                });
            }
        });
    ...
    $(".shipping-options div").click(function (e) {
        ...
        var $shipping_method_wrapper = $shipping_method.closest(
            ".wpgv-form-fields"
        );
        if ($(this).data("value") == "shipping_as_post") {
            ...
            $shippingpricespan.html(
                $shipping_method_wrapper.find(":nth-child(2)").data("value")
            );
            $shipping_method_wrapper
                .find(":nth-child(2) input")
                .prop("checked", true);
            var $totalprice = (
                parseFloat($itempricespan.html()) +
                parseFloat($shippingpricespan.html())
            ).toFixed(2);
            ...
            $shippingbox.hide();
            $totalbox.hide();
            // $wpgv_total_price.val($itempricespan.text());
            // $('.voucherValueCard').val($itempricespan.text());
        }
    });

    $('input[name="shipping_method"]').change(function () {
        var $shippingprice = $(this)
            .closest("label")
            .data("value");
        var $totalprice = (
            parseFloat($itempricespan.html()) + parseFloat($shippingprice)
        ).toFixed(2);
    ...
    $("#paynowbtn").click(function () {
        ...
        if ($shipping.val() == "shipping_as_email") {
            if (
                $buying_for.val() == "someone_else" &&
                !($shipping_email.val() && wpgv_validateEmail($shipping_email.val()))
            ) {
                $error = 1;
                $shipping_email
                    .closest(".wpgv-form-fields")
                    .find(".error")
                    .show();
            } else {
                $shipping_email
            ...
            $datastring =
                ...
                $urlstring +
                "&shipping=" +
                wpgv_b64EncodeUnicode($shipping.val()) +
                "&shipping_email=" +
                wpgv_b64EncodeUnicode($shipping_email.val()) +
                "&receipt_email=" +
                wpgv_b64EncodeUnicode($receipt_email.val()) +
                "&paymentmethod=" +
                wpgv_b64EncodeUnicode($payemnt_gateway.val());
            ...
            $datastring =
                ...
                wpgv_b64EncodeUnicode($receipt_email.val()) +
                "&firstname=" +
                wpgv_b64EncodeUnicode($post_firstname.val()) +
                "&lastname=" +
                wpgv_b64EncodeUnicode($post_lastname.val()) +
                "&address=" +
                wpgv_b64EncodeUnicode($post_address.val()) +
                "&pincode=" +
                wpgv_b64EncodeUnicode($post_code.val()) +
                "&shipping_method=" +
                wpgv_b64EncodeUnicode($("input[name=shipping_method]:checked").val()) +
                "&paymentmethod=" +
                wpgv_b64EncodeUnicode($payemnt_gateway.val());
        }
    ...
    $("#wpgv-message #message").on("keydown", function (e) {
        newLines = $(this)
            ...
            .split("\n").length;
        $(".maxchar").html(
            frontend_ajax_object.total_character + ": " + this.value.length
        );
        if (
            (e.keyCode == 13 && newLines >= 3) ||
            (e.keyCode != 8 && this.value.length > 250)
        ) {
            return false;
        }
    });
